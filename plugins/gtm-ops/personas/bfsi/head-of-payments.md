...
1. **Bureau prefill misses 50-60% of NTC (New-to-Credit) applicants.** Their lender book can't grow without expanding NTC coverage. Currently using bureau-only stack (CIBIL/CRIF/Experian) → 60M reachable. Cashfree Mobile360 takes them to 190M+. **This is the #1 wedge.**
...
3. **RBI alternate-data restrictions tightening.** "Signals not scores" rule + DPDP enforcement means many vendors must re-architect. Most BFSI heads of payments are scrambling for compliant alternatives. Cashfree Secure ID's "DPDP-native, signals-not-scores" architecture is a compliance-first wedge.
...
- Cashfree-managed POC (5 days vs Karza's 1 week)
...
- **POC offers**: "Cashfree-managed POC — our SE runs your sample file, delivers benchmark deck in 5 days, zero load on your team"
...
| **"HyperVerge is faster on POC"** | "We've matched. Cashfree-managed POC: 5 days, our SE owns the benchmark, zero engineering load on your team. Run apples-to-apples vs HyperVerge POC and decide." |
...
| Hiring signal | "Saw {bank} hired 2 Senior Onboarding PMs on LinkedIn. Usually means stack re-evaluation. We've formalized a Cashfree-managed POC — 5 days, zero engineering load. Tuesday 4pm IST?" |
| NTC coverage gap | "Bureau prefill misses 60% of NTC applicants. Mobile360 takes you to 190M coverage from single mobile-number input. {Peer-bank} unlocked {X}% additional approvals. Worth a Cashfree-managed POC on 1000 of your declined applications?" |
...
| Karza renewal | "Karza renewal usually comes around the {date}. Worth a 20-min benchmark discussion before then? We can have a Cashfree-managed POC ready inside your renewal window." |
...
| Cold email (Cashfree-warmed domain) | Backup | 2 emails 5d apart |
...
Continuous: `cf-drive-transcript-extractor` updates this file as real BFSI calls accumulate
